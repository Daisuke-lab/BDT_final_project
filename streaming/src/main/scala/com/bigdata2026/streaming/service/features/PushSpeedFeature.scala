...
final class PushSpeedFeature(hbaseConfig: HBaseConfig) extends StreamFeature {
  val name = "push-speed"

  private val TableName_ = "push_speed_per_window"
...
object PushSpeedFeature {
  val live: URLayer[HBaseConfig, PushSpeedFeature] =
    ZLayer.fromZIO(ZIO.service[HBaseConfig].map(new PushSpeedFeature(_)))
}
