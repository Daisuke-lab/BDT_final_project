...
final class RepoStatsFeature extends Feature:
  ...
  def liveUpdates: ZStream[Any, Throwable, ServerMsg] =
    ZStream.fromZIO(currentMaxLastSeen).flatMap { initSince =>
      ZStream
        ...
        .mapAccumZIO(initSince) { (lastSeen, _) =>
          ZIO.attemptBlocking {
            withTable { table =>
              ...
              val newMax  = repos.map(_.lastSeen).maxOption.getOrElse(lastSeen)
              (newMax, if (newMax > lastSeen) repos else Nil)
            }
          }
        }
        .flatMap { repos =>
  ...
  private def currentMaxLastSeen: Task[Long] =
    ZIO.attemptBlocking {
      withTable { table =>
        val scanner = table.getScanner(new Scan())
        try parseRows(scanner).map(_.lastSeen).maxOption.getOrElse(0L)
        finally scanner.close()
      }
    ...
    Iterator.continually(scanner.next())
      ...
      .map { row =>
        RepoStats(
          ...
          firstSeen        = getLong(row, "first_seen"),
          lastSeen         = getLong(row, "last_seen"),
        )
      }
      .toList
